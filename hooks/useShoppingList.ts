...
export function useShoppingList() {
  ...
  const toggleMutation = useMutation(
    api.shoppingList.toggleShoppingListItemChecked
  );
  ...
  const localAddMultipleItems = useShoppingListStore(
    (state) => state.addMultipleItems
  );
  const localRemoveItem = useShoppingListStore((state) => state.removeItem);
  const localToggleItem = useShoppingListStore(
    (state) => state.toggleItemChecked
  );
  const localClearChecked = useShoppingListStore(
